...
router.route("/inventoryReport").get(async (req, res) => {
  try {
    ...
    const combinedResults = allCart.map((cartItem) => {
      const detailItem = allCartDetails.find(
        (detail) =>
          detail.product_inventory_id === cartItem.product_inventory_id
      );

      const productInventory = detailItem ? detailItem.product_inventory : null;
      ...
      return {
        ...
        total_quantity: cartItem.get("total_quantity"),
      };
    });
