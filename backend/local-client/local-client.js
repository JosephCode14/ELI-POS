    ...
    if (message) {
      try {
        ...
        if (printerStatus.includes("reachable")) {
          ...
          console.log("Cashieer", cashier);
          if (!cashier) {
            printLongText(message);
          }

          const { cartWithoutImages } = parsedMessage;
          const { eachVariantPrice } = cartWithoutImages;

          const printableItems = cartWithoutImages.filter(
            (item) => item.printable
          );
          console.log(cartWithoutImages);
          console.log("Variant", eachVariantPrice);
...
function printLongText(receivedMessage) {
  ...
  const initialBalance = studentBalance.balance.toString();
  const remainingBalance = initialCompute.toString();

  const total = totalOrder.toString();
  const tapCard = totalOrder.toString();
  const tendered = totalOrder.toString();
  const change = "0";
...
function printKitchenReceipt(data) {
