...
const BulkAdd = ({
  ...
  const create_credits_approver = async (e) => {
    e.preventDefault();
    ...
    if (form.checkValidity() === false) {
      ...
      swal({
        ...
        if (confirmed) {
          ...
          axios
            .post(`${BASE_URL}/credits/createCreditsApprover`, {
              userId,
              credits,
            })
            .then((res) => {
  ...
  return (
    <>
      <Modal
        show={bulkEditModal}
        onHide={() => setBulkEditModal(false)}
        size="xl"
      >
        <Form onSubmit={create_credits_approver}>
          <Modal.Header>
            <Modal.Title>
              <div className="d-flex flex-direction-row align-items-center p-0">
                <h2>Create Bulk Credits</h2>
              </div>
            </Modal.Title>
          </Modal.Header>
