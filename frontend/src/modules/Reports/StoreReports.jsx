...
const StoreReports = ({ authrztn }) => {
  ...
  const operateColumn = [
    {
      name: "OPEN DATE",
      selector: (row) => formatDateOnly(row.store_open),
    },
    {
      name: "CLOSE DATE",
      selector: (row) => formatDateOnly(row.store_close),
    },
    {
      name: "TOTAL SALES",
  ...
  const handleExport = (format) => {
    ...
    if (format == "excel") {
      const exportedData = filteredTransactions.map((item) => ({
        "OPEN DATE": formatDateOnly(item.store_open),
        "CLOSE DATE": formatDateOnly(item.store_close),
      ...
      const tableData = filteredTransactions.map((item) => [
        formatDateOnly(item.store_open),
        formatDateOnly(item.store_close),
        `${item.total_sales}`,
